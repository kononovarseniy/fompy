...
наименования единицы. Этот суффикс выглядит так: `^`/отсутствие символа + знак (`-` для отрицательной степени, 
`+`/отсутствие символа для положительной степени) + целое число. Пробелы между знаком и числом запрещены.

Примеры: микрометр в минус третьей степени &mdash; `um^-3` или `um-3`; вольт &mdash; `V`, или `V ^1`, или `V ^ +1`.
